...
namespace UnityEngine.UI.Extensions
{
    ...
    public class HorizontalScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        void Start()
        {
            Debug.Log("parent: " + parentNode.sizeDelta);
            ...
            if (PageStep == 0)
            {
                PageStep = 1080;// (int)_scroll_rect.GetComponent<RectTransform>().rect.width * 3;
            }
            DistributePages();
        ...
        private void DistributePages()
        {
            int _offset = 0;
            int _dimension = 0;
            RectTransform currentRtf = gameObject.GetComponent<RectTransform>();
            Vector2 panelDimensions = new Vector2(1080, currentRtf.rect.height);// gameObject.GetComponent<RectTransform>().sizeDelta;
            int currentXPosition = 0;

            for (int i = 0; i < _screensContainer.transform.childCount; i++)
            {
                RectTransform child = _screensContainer.transform.GetChild(i).gameObject.GetComponent<RectTransform>();
                currentXPosition = 540 + _offset + i * PageStep;                
            ...
            Debug.LogError("dimension: " + _dimension);
            ...
            if (_screens > 0)
            {
                ...
                {
                    _scroll_rect.horizontalNormalizedPosition = (float) i / (_screens - 1);
                    Debug.Log("lít post: " + _screensContainer.localPosition);

                    RectTransform child = _screensContainer.transform.GetChild(i).gameObject.GetComponent<RectTransform>();
                    child.anchoredPosition = _screensContainer.localPosition;
                    child.offsetMin = new Vector2(0.5f, 0.5f);
                    child.offsetMin = new Vector2(0.5f, 0.5f);
