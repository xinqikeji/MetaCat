...
public class BulletCircle2 : BulletBase
{
    ...
    public void Setup(Team team, EntityBase attacker, BulletParam param, EntityBase target)
    {
        ...
        var beastAttacker = attacker as BeastBase;
        var beastTarget = target as BeastBase;

        float x = 4 + 0.7f * (beastAttacker.curStar - 2);
        float ratio = beastAttacker.curStar / beastTarget.curStar;
        x *= ratio <= 1 ? ratio : 1;

        beastTarget.ConvertTeam(beastAttacker, x);
    }
